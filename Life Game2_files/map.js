var Map = function(canvasId, lines, columns, livingCells) {
	...
	this.allExecute = function() {
		...
		for ( p = 0; p < this.lines; ++p) {
			for ( q = 0; q < this.columns; ++q) {
				...
				if (cell["currentStatus"] == 1) {//死亡
					if ((livingCellsAround < 2 || livingCellsAround > 3) && cell["hp"] > 1) {
						cell["hp"] -= 5;
				...
				} else if (cell["currentStatus"] == 0 && livingCellsAround == 3 && livingCellsLifeAround > 270) {//复活
					cell["hp"] = 100;
					this.dynamicArray.push(cell);
				}
			}
	...
	this.scaleMap = function(scale) {
		...
		*/
		...
		// mouseMove = false;
		// }
		if (scale > 0 && this.scale < this.mapSize / 10000) {
			this.scale += 0.1
		} else if (scale < 0) {
			if (this.scale > 1) {
				this.scale -= 0.1;
		...
		this.render();
		// mouseMove = true;
		$("scale").innerHTML = this.scale + "/" + this.mapSize / 10000;
	}
	this.quick = function() {
		if (this.speed > 50) {
			this.loop = false;
	...
	this.render = function() {
		...
		while (i >= 0) {
			this.context.globalAlpha = this.dynamicArray[i]["hp"] / 100;
			this.context.rect((this.dynamicArray[i].position[1]) * this.cellWidth + this.translateX, this.dynamicArray[i].position[0] * this.cellHeight + this.translateY, this.cellWidth, this.cellHeight);
			--i;
		}
		this.context.fill()
		// if (this.mapSize < 100000) {
